...
def power_law_transformation(negative_image):
    gamma = 0.1
    power = numpy.power(negative_image, gamma)
    power = cv2.normalize(power, None, 0, 255, cv2.NORM_MINMAX)

    return numpy.uint8(power)

def log_transformation(negative_image):
    log_transform = cv2.normalize(numpy.array(numpy.log(1 + negative_image), dtype=numpy.float32), None, 0, 255, cv2.NORM_MINMAX)
